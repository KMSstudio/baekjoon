...
void solve(void){
    ...
    sot(a); cin >> x;
    lft = lower_bound(a.begin(), a.end(), x/2) - a.begin();
    rht = upper_bound(a.begin(), a.end(), x/2) - a.begin();
    cnt = 0;

    for(int i=0;i<n;i++){
        lft = lower_bound(a.begin(), a.end(), x - a[i]) - a.begin();
        rht = upper_bound(a.begin(), a.end(), x - a[i]) - a.begin();
        if(lft != rht){ cnt++; }
    }
